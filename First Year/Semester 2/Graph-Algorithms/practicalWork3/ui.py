...
class Ui:
    def __init__(self):
        self.__allCommands = {
            ...
            '20' : self.findLowestPathBetweenTwoVertices,
            '21' : self.findLowestCostWalkBetweenTwoVertices
        }
        self.__listOfGraphs = []
        self.__indexOfCurrentGraph = None


    def findLowestPathBetweenTwoVertices(self):
        sourceVertex = int(input("Enter source vertex: "))
        targetVertex = int(input("Enter target vertex: "))
        result = self.__listOfGraphs[self.__indexOfCurrentGraph].findLowestPathUsingForwardBreadthFirst(sourceVertex, targetVertex)
        if result:
            distance, shortestPath = result
            print(f"Shortest path from {sourceVertex} to {targetVertex}:", *shortestPath)
            print(f"Length of shortest path: {distance}")
        else:
            print(f"No path found from {sourceVertex} to {targetVertex}")

    def findLowestCostWalkBetweenTwoVertices(self):
        sourceVertex = int(input("Enter source vertex: "))
        targetVertex = int(input("Enter target vertex: "))
        cost, path = self.__listOfGraphs[self.__indexOfCurrentGraph].findLowestCostWalkUsingMatrixMultiplication(sourceVertex, targetVertex)

        if cost != float('inf'):
            print(f"Lowest cost walk from {sourceVertex} to {targetVertex}:", *path)
            print(f"Cost of walk: {cost}")
        else:
            print(f"No walk found from {sourceVertex} to {targetVertex}")
    ...
    def displayMenu():
        ...
        print("\t20. Find lowest length path between two given vertices")
        print("\t21. Find lowest cost walk between two given vertices")
        print("\t0. Exit")
