from graph import UndirectedGraphDictionaryRepresentation, readGraphFromTextFile, writeGraphToTextFile, createRandomGraph
...
class Ui:
    def __init__(self):
        self.__allCommands = {
            ...
            '9' : self.parseSetOfEdgesOfAVertex,
            '10' : self.getTheCostOfASpecifiedEdge,
            '11' : self.changeTheCostOfASpecifiedEdge,
            '12': self.createRandomGraph,
            '13' : self.createACopyOfTheGraph,
            '14' : self.readGraphFromTextFile,
            '15' : self.writeGraphToTextFile,
            '16' : self.switchCurrentGraph,
            '17' : self.kruskalAlgorithm
        }
        self.__listOfGraphs = []
    ...
    def kruskalAlgorithm(self):
        mst = self.__listOfGraphs[self.__indexOfCurrentGraph].kruskalMST()
        for u, v, w in mst:
            print(f"{u} - {v}: {w}")
    ...
    def createAnEmptyGraph(self):
        emptyGraph = UndirectedGraphDictionaryRepresentation()
    ...
    def displayMenu():
        ...
        print("\t9. Parse the set of  edges of a vertex")
        print("\t10. Get the cost of a specified edge")
        print("\t11. Change the cost of a specified edge")
        print("\t12. Create a random graph with specified number of vertices and of edges")
        print("\t13. Make a copy of the graph")
        print("\t14. Read a graph from a text file")
        print("\t15. Write the graph in a text file")
        print("\t16. Switch between existing graphs")
        print("\t17. Find the minimum cost spanning tree of the graph using Kruskal Algorithm")
        print("\t0. Exit")
