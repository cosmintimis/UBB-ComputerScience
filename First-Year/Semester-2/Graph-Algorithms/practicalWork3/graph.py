...
class DirectedGraphDictionaryRepresentation:
    ...
    @staticmethod
    def constructPath(startVertex, endVertex, Next, setOfVertices):

        indexOfStartVertex = setOfVertices.index(startVertex)
        indexOfEndVertex = setOfVertices.index(endVertex)
        if Next[indexOfStartVertex][indexOfEndVertex] == -1:
            return []

        path = [startVertex]
        while startVertex != endVertex:
            startVertex = Next[indexOfStartVertex][indexOfEndVertex]
            indexOfStartVertex = setOfVertices.index(startVertex)
    ...
    def findLowestCostWalkUsingMatrixMultiplication(self, sourceVertex, targetVertex):
        # Check if the source vertex exists in the graph
        if not self.checkIfTheGraphHasGivenVertex(sourceVertex):
            raise GraphError("Given vertex doesn't exist in current graph!")

        # Check if the target vertex exists in the graph
        if not self.checkIfTheGraphHasGivenVertex(targetVertex):
            raise GraphError("Given vertex doesn't exist in current graph!")


        length = self.getNumberOfVertices
        initialMatrix = [[0 for _ in range(length)] for _ in range(length)]
        Next = [[-1 for _ in range(length)] for _ in range(length)]

        setOfVertices = list(self.parseSetOfVertices())
        setOfVertices.sort()

        # Initialize the initial matrix and Next matrix
        for i in range(length):
            start = setOfVertices[i]
            for j in range(length):
                end = setOfVertices[j]
                if start is not end:
                    if (start, end) in self.__dictionaryCost.keys():
                        initialMatrix[i][j] = self.__dictionaryCost[(start, end)]
                        Next[i][j] = end
                    else:
                        initialMatrix[i][j] = float('inf')


        print('\n', initialMatrix)
        newMatrix = deepcopy(initialMatrix)

        # Perform matrix multiplication and update the new matrix iteratively
        for _ in range(2, length):
            newMatrix = self.extend(newMatrix, initialMatrix, Next)
            print('\n', newMatrix)

        # Check for negative cost cycles
        for i in range(length):
            if newMatrix[i][i] < 0:
                raise GraphError("Graph contains negative cost cycles.")

        # Construct the actual path from sourceVertex to targetVertex
        actualWalk = self.constructPath(sourceVertex, targetVertex, Next, setOfVertices)

        indexOfSourceVertex = setOfVertices.index(sourceVertex)
        indexOfTargetVertex = setOfVertices.index(targetVertex)

        # Return the lowest cost and the actual walk
        return newMatrix[indexOfSourceVertex][indexOfTargetVertex], actualWalk


    def findLowestPathUsingForwardBreadthFirst(self, sourceVertex, targetVertex):
        if not self.checkIfTheGraphHasGivenVertex(sourceVertex):
            raise GraphError("Given vertex doesn't exist in current graph!")
        ...
        queueThatStoresVerticesAndCorrespondingDistance = deque([(sourceVertex, 0)])
        predecessors = {}
        setOfVisitedVertices = {sourceVertex}

        # Perform breadth-first search
        while queueThatStoresVerticesAndCorrespondingDistance:
            currentVertex, distance = queueThatStoresVerticesAndCorrespondingDistance.popleft()

            # Check if target vertex has been reached
            if currentVertex == targetVertex:
                break

            # Explore neighboring vertices
            for neighbourVertex in self.parseSetOfOutboundEdgesOfAVertex(currentVertex):
                if neighbourVertex not in setOfVisitedVertices:
                    queueThatStoresVerticesAndCorrespondingDistance.append((neighbourVertex, distance + 1))
                    setOfVisitedVertices.add(neighbourVertex)
                    predecessors[neighbourVertex] = currentVertex

        # If target vertex is reached, construct the lowest length path
        if targetVertex in predecessors:
            shortestPath = []
            while targetVertex != sourceVertex:
                shortestPath.append(targetVertex)
                targetVertex = predecessors[targetVertex]
            shortestPath.append(sourceVertex)
