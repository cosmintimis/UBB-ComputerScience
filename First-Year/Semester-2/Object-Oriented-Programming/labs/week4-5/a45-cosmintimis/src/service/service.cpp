...
int Service::searchForEvent(std::string& title, int& numberOfPeople)
{
    ...
    {
        Event currentEvent = getAllEvents().getElemByIndex(i);
...
{
    return getAllEvents().getElemByIndex(index);
}

DynamicVector<Event>& Service::getAllEvents()
{
...
void Service::getEventsForAGivenMonthOrderedChronologically(int month, DynamicVector<Event>& eventsForAGivenMonthOrderedChronologically)
{
    ...
    DynamicVector<Event> allEvents = this->getAllEvents();
    if(month == 0)
    {
        for(int i = 0; i < allEvents.getSize(); i++)
            eventsForAGivenMonthOrderedChronologically.addElemToVector(allEvents.getElemByIndex(i));
    ...
    else
    {
        for(int i = 0; i < allEvents.getSize(); i++)
        {
            if(allEvents.getElemByIndex(i).getDateAndTime().tm_mon == month - 1)
                eventsForAGivenMonthOrderedChronologically.addElemToVector(allEvents.getElemByIndex(i));
            
        }
    }

    for(int i = 0; i < eventsForAGivenMonthOrderedChronologically.getSize() - 1; i++)
    {
        for(int j = i + 1; j < eventsForAGivenMonthOrderedChronologically.getSize(); j++)
        {
            if(compareTwoDates(eventsForAGivenMonthOrderedChronologically.getElemByIndex(i).getDateAndTime(), eventsForAGivenMonthOrderedChronologically.getElemByIndex(j).getDateAndTime()))
                eventsForAGivenMonthOrderedChronologically.swapTwoElements(i, j);
        }
    }
}
...
int Service::addEventToUserList(std::string& eventTitle, int& numberOfPeople)
{
    ...
    for(int i = 0; i < this->repository.getUserList().getSize(); i++)
    {
        Event currentEvent = this->repository.getUserList().getElemByIndex(i);
...
DynamicVector<Event> &Service::getUserList()
{
