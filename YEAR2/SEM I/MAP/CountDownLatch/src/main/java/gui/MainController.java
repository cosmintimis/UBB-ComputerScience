...
public class MainController {
    ...
    private void initLatchTable(){
        ProgramState selectedProgramState = getCurrentProgramState();

        if(selectedProgramState != null){
            MyILatchTable latchTbl = selectedProgramState.getLatchTable();
            latchTable.setItems(FXCollections.observableArrayList(latchTbl.getContent().entrySet().stream().map(item -> new Pair<Integer, Integer>(item.getKey(), item.getValue())).toList()));
        }
        else
