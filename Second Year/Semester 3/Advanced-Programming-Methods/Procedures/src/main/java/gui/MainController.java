...
public class MainController {
    ...
    private TableView<Map.Entry<String, Pair<List<String>, IStmt>>> procTable;

    @FXML
    private TableColumn<Map.Entry<String, Pair<List<String>, IStmt>>, Pair<String, List<String>>> procNameAndParamColumn;

    @FXML
    private TableColumn<Map.Entry<String, Pair<List<String>, IStmt>>, String> procBodyColumn;
    ...
    public void initialize(){
        ...
        procNameAndParamColumn.setCellValueFactory(p -> new SimpleObjectProperty<>(new javafx.util.Pair<String, List<String>>(p.getValue().getKey(), p.getValue().getValue().getKey())));
        procBodyColumn.setCellValueFactory(p -> new SimpleStringProperty(p.getValue().getValue().getValue().toString()));
    ...
    private void initSymbolTable(){
        ...
        if(selectedProgramState != null){
            MyIDictionary<String, IValue> symTbl = selectedProgramState.getTopOfSymTable();
    ...
    private void initProcTable(){
        MyIProcTable procTbl = getCurrentProgramState().getProcTable();

        procTable.setItems(FXCollections.observableList(
                procTbl.getContent().entrySet().stream()
